...
build() {
    ...
    makew -C "$PACKAGE_BSCRIPT_DIR" default CFLAGS="'$CFLAGS $CPPFLAGS $LDFLAGS -I$SOURCE_DIR/src'" AR="$AR" RANLIB="$RANLIB" &&
    install -d "$PACKAGE_INCLUDE_DIR" &&
    install -d "$PACKAGE_LIBRARY_DIR" &&
    makew -C "$PACKAGE_BSCRIPT_DIR" install prefix="$PACKAGE_INSTALL_DIR"
}
